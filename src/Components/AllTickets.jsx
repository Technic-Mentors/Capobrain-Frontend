...
export default function AllTickets() {
    ...
    const editTicket = async (id) => {
        const res = await fetch(`https://capobrain-backend.vercel.app/api/auth/getTicket/${id}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
    ...
    const updateStatus = async (e) => {
        ...
        if (isConfirmed) {
            await fetch(`https://capobrain-backend.vercel.app/api/auth/updateTicket/${ediitTicket._id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json"
                },
    ...
    const sendMessage = async (e) => {
        ...
        const res = await fetch("https://capobrain-backend.vercel.app/api/auth/createMessage", {
            method: 'POST',
            headers: {
                "Content-Type": "application/json"
            },
                            ...
                            {/* <div className="card" style={{ display: "inline-block" }}>
                                <div className="card-body">
                                    <div className="d-flex align-items-center">
                                        <h4>Ticket Title :</h4> <h6 className='ml-2' style={{ color: "initial" }}>{ediitTicket.title}</h6>
                                    </div>
                                    <div className="d-flex align-items-center">
                                        <h4>Ticket Id :</h4> <h6 className='ml-2' style={{ color: "initial" }}>{ediitTicket.ticketId}</h6>
                                    </div>
                                    <div className="d-flex align-items-center">
                                        <h4>Email :</h4> <h6 className='ml-2' style={{ color: "initial" }}>{ediitTicket.userId && ediitTicket.userId.email}</h6>
                                    </div>
                                    <div className="d-flex align-items-center">
                                        <h4>Subject :</h4> <h6 className='ml-2' style={{ color: "initial" }}>{ediitTicket.subject}</h6>
                                    </div>
                                    <div className="d-flex align-items-center">
                                        <h4>Severity :</h4> <h6 className='ml-2' style={{ color: "initial" }}>{ediitTicket.severity}</h6>
                                    </div>
                                    <div className="d-flex align-items-center">
                                        <h4>Message:</h4> <h6 className='ml-2' style={{ color: "initial" }}>{ediitTicket.message}</h6>
                                    </div>
                                </div>
                            </div>
                            <div>
                                <h1 className='text-center mt-3'>Messages</h1>
                                <div className='mb-3 card messageCard'>
                                    <div className="card-body">
                                        {allMessages && allMessages.filter(message => message.ticketId.ticketId === ediitTicket.ticketId).map((message) => {
                                            return <div key={message._id}> <p className={`text-${message.messageStatus === "Admin Message" ? "end" : "start messageDisplay"}`}>
                                                {message.message}</p>
                                            </div>
                                        })}
                                    </div>
                                    <div className="d-flex messages">
                                        <input type="text" name="message" className='form-control' placeholder='Type Message Here' value={message} onChange={(e) => setMessage(e.target.value)} />
